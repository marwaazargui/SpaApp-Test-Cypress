...
describe("Staff Profile Page Validation", () => {
  ...
  const getFormattedTime = (hour24: number) => {
  ...
  beforeEach(() => {
    ...
    cy.wait("@getStaff").then((interception) => {
      ...
      staffId = interception.response?.body?.data?.[0]?.id;
  ...
  it("should display correct staff profile details from the API", () => {
    cy.intercept("GET", `${apiUrl}/Staff/*`).as("getSpecificStaff");

    openFirstStaffProfile();

    cy.wait("@getSpecificStaff").then((interception) => {
      if (!interception.response?.body) throw new Error("Invalid API response");
      ...
      if (staffData.availability?.length > 0) {
        ...
        const expectedText = `${getFormattedTime(
          localFromTime
        )} - ${getFormattedTime(localToTime)}`;
      ...
      cy.contains(
        "div.text-foreground",
        `Team member since ${formatDate(staffData.created)}`
      ).should("be.visible");
    });
  });
