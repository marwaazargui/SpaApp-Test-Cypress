...
function canadianPostalCode() {
  ...
  return `${letters[Math.floor(Math.random() * 26)]}${digits[Math.floor(Math.random() * 10)]}${letters[Math.floor(Math.random() * 26)]} ${digits[Math.floor(Math.random() * 10)]}${letters[Math.floor(Math.random() * 26)]}${digits[Math.floor(Math.random() * 10)]}`;
}

// Generate a Canadian address object
function fakeCanadianAddress() {
  return {
    street: `${Math.floor(Math.random() * 9999) + 1} ${defaultFaker.location.street()}`, // fallback street
    suite: Math.random() > 0.5 ? `Suite ${Math.floor(Math.random() * 999) + 1}` : '',
